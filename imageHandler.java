...
public class imageHandler{
   ...
   public void drawMap(int[][] map,Graphics2D g,int X,int Y,int sz){
      ...
      if(backWallSwitch){
         for(int i = 0;i<map[0].length;i++){
            for(int k = 0;k<map.length;k++){
               if((map[k][i] == 0||map[k][i] == 7||map[k][i] == 27)&&Math.abs(x+FRAME1/2-i*size)<FRAME1/2+size && Math.abs(y+FRAME2/2-k*size)<FRAME2/2+size){
      ...
      for(int i = 0;i<map[0].length;i++){
         for(int k = 0;k<map.length;k++){
            //Normal Wall Block
            if(map[k][i] == 1&&Math.abs(x+FRAME1/2-i*size)<FRAME1/2+size && Math.abs(y+FRAME2/2-k*size)<FRAME2/2+size){
